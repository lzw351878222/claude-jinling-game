...
- **消消乐**：拖动（或先后点选）两颗相邻的棋子交换，三个相同即可消除。
  ...
  - **省步有赏**：过关时剩的步数越多，星越多、分越高——剩四分之一以上的步数过关得三星；每省一步另加一份「余步奖励」，所以用的步越少，总分越高
  - 步数用完时，阿麟可以借你五步（每局一次，借步过关只记一星）；还有「木槌」「乾坤换」两件道具
...
```bash
node tools/m3-levels-sim.mjs 200     # 用贪心机器人把 30 关各跑 200 局，检查不变量、难度与"省步得高分"
